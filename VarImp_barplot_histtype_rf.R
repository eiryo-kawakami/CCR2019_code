...
mycols <- brewer.pal(5,"Set1")[c(1,2,3,5)]

df <- read.table('ovary_rf_importance_summary.txt',header=TRUE,row.names=1)

df_top <- df[head(rev(order(rowSums(df[,c(3:6)]))),top_num),]

df_top <- data.frame(Var=factor(rownames(df_top),levels=rownames(df_top)),df_top[,c(3:6)])
data <- melt(df_top)
...
ggsave("ovary_histtype_importance_rf.pdf",width=5,height=2)
