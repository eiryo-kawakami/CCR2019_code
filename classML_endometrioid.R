...
#label <- c("clear\ncell","serous","endo\nmetrioid","mucinous")
order <- c("endometrioid","non_endometrioid")
label <- c("endometrioid","non_endometrioid")
...
convert_class <- function(vec){
	...
	for (val in vec){
		if (val == "endometrioid"){
			tmp_vec <- c(tmp_vec,"endometrioid")
		} else if (val == "others" | val == "cyst" | val == "teratoma") {
			tmp_vec <- c(tmp_vec,NA)
		} else {
			tmp_vec <- c(tmp_vec,"non_endometrioid")
		}
	}
...
for (method in method_list){
	...
	if (class(e) != "try-error") {
		...
		roc.res <- roc(data_test_merged$class, pred$endometrioid)
