...
project <- "ovary"
class <- "cancer_age_single_171122"
margin <- 1
...
for (val in use_exp_vars){
	...
	data_train_merged_ind <- data_train_merged[,c(val,"class")]
	data_test_merged_ind <- data_test_merged[,c(val,"class")]
	...
	for (method in method_list){
		...
		if (method != "glm"){
			...
			e <- try(model <- train(class ~ ., data = data_train_merged_ind, 
		         method = method, 
		...
		if (class(e) != "try-error") {
			...
			pred<-predict(model, data_test_merged_ind, type="prob")
			roc.res <- roc(data_test_merged_ind$class, pred$malignant)
			auc_list <- c(auc_list,pROC::auc(roc.res))
			rocplot <- paste("../results_171122/ovary_ROCplot_",method,"_",val,"_",k,"-fold_",class,".pdf",sep='')
			...
			dev.off()
			pred<-predict(model, data_test_merged_ind)
			res <- table(data_test_merged_ind[,"class"],pred)
			#print(res)
			res2 <- filltable(res)
			print(calc_accuracy(res2))
			accuracy_list <- c(accuracy_list,calc_accuracy(res2))

			#print(res2)
			colorvec <- setcolor(res2)
			blnplot <- paste("../results_171122/ovary_balloonplot_",method,"_",val,"_",k,"-fold_",class,".pdf",sep='')
			...
			# if (class(e) != "try-error") {
			# 	imp_file <- paste("../results_171122/",project,class,"_",method,"importance.txt",sep='')
...
acc_result <- data.frame(method=use_exp_vars,accuracy=accuracy_list,auc=auc_list)
acc_result_file <- paste("../results_171122/",class,"accuracy_",k,"-fold_validation.txt",sep="")
