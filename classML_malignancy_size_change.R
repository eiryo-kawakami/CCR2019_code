...
for (method in method_list){
	...
	for (i in c(1:5)){
		...
		for (j in c(1:10)){
			...
			train_sample <- sample(1:nrow(data_train_merged),size=round(nrow(data_train_merged)*i/5),replace=F)
			data_train_sample <- data_train_merged[train_sample,]
			test_sample <- sample(1:nrow(data_test_merged),size=round(nrow(data_test_merged)*i/5),replace=F)
			data_test_sample <- data_test_merged[test_sample,]
			...
			if (method != "glm"){
				...
				e <- try(model <- train(class ~ ., data = data_train_sample,
			         method = method,
			...
			if (class(e) != "try-error") {
				...
				pred<-predict(model, data_test_sample, type="prob")
				roc.res <- roc(data_test_sample$class, pred$malignant)
				auc_list <- c(auc_list,pROC::auc(roc.res))
				rocplot <- paste("../results_171122/ovary_ROCplot_",method,"_",k,"-fold_",class,"_",i*20,"_epoch",j,".pdf",sep='')
				...
				dev.off()
				pred<-predict(model, data_test_sample)
				res <- table(data_test_sample[,"class"],pred)
				#print(res)
				res2 <- filltable(res)
		...
		auc_res <- cbind(auc_res,auc_list)
		accuracy_res <- cbind(accuracy_res,accuracy_list)

	}
}

auc_res <- data.frame(auc_res)
rownames(auc_res) <- c("epoch1","epoch2","epoch3","epoch4","epoch5","epoch6","epoch7","epoch8","epoch9","epoch10")
colnames(auc_res) <- c("20%","40%","60%","80%","100%")
auc_result_file <- paste("../results_171122/",class,"auc_",k,"-fold_validation_size_change.txt",sep="")
write.table(auc_res,file=auc_result_file,sep="\t",quote=FALSE)

accuracy_res <- data.frame(accuracy_res)
rownames(accuracy_res) <- c("epoch1","epoch2","epoch3","epoch4","epoch5","epoch6","epoch7","epoch8","epoch9","epoch10")
colnames(accuracy_res) <- c("20%","40%","60%","80%","100%")
accuracy_result_file <- paste("../results_171122/",class,"accuracy_",k,"-fold_validation_size_change.txt",sep="")
write.table(accuracy_res,file=accuracy_result_file,sep="\t",quote=FALSE)
